...
**`ontology/example-flibe.ttl`** (worked A-Box, 46 triples).
...
# a LiF-BeF2 melt at the exact composition of a verified NIST density row (34 mol% LiF, 66 mol% BeF2)
msrd:LiF-BeF2_34-66 a msr:MoltenSalt ; rdfs:label "LiF-BeF2 (34-66 mol%)" ;
    msr:hasConstituent [ msr:ofCompound msrd:LiF  ; msr:moleFraction 0.34 ] ,
                       [ msr:ofCompound msrd:BeF2 ; msr:moleFraction 0.66 ] ;
    skos:closeMatch voc:flibe .

# its density: NIST row  `LiF-BeF2,34.0-66.0,P1,800,1080,,2.413,-4.88E-4`  →  ρ(T)=2.413 − 4.88e-4·T
msrd:m-LiFBeF2-3466-density a msr:PropertyMeasurement ;
    msr:ofSalt msrd:LiF-BeF2_34-66 ;
    ...
    msr:dataLocator "nist-srd27/density#LiF-BeF2|34.0-66.0" ;
    prov:wasDerivedFrom msrd:nist-srd27 ;
    msr:citedIn msrd:ORNL-TM-2316 .

# MSRE and its coolant salt (canonical FLiBe 66-34); its exact NIST row confirmed on parse
msrd:MSRE a msr:MoltenSaltReactor ; rdfs:label "MSRE" ; skos:closeMatch voc:msre-reactor .
msrd:msre-coolant a msr:MoltenSalt ; rdfs:label "MSRE coolant salt (FLiBe 66-34)" ;
    msr:hasConstituent [ msr:ofCompound msrd:LiF  ; msr:moleFraction 0.66 ] ,
...
|-------------|----|----|----|----|
| `nist-srd27/density#LiF-BeF2\|34.0-66.0` | 2.413 | -4.88e-4 | | |
...
1. **SPARQL the graph** → find the measurement for the salt: property `density`, unit
   ...
   locator `nist-srd27/density#LiF-BeF2|34.0-66.0`.
...
SELECT ?prop ?unit ?form ?tmin ?tmax ?locator WHERE {
  ?m msr:ofSalt msrd:LiF-BeF2_34-66 ;
