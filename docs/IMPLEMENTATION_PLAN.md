...
- **SQLite runtime:** journal mode `DELETE` (WAL would break the sandboxes' read-only
  ...
  read-only (journal sidecars stay visible), checkpoints copy via `VACUUM INTO` on a
...
- **Delivered:** the 11.4.2 config surface is confirmed live (sail-type literal `rdf4j:ShaclSail`, not the `graphdb:ShaclSail` guess — GraphDB 400s on the latter; see design D1 and `deploy/graphdb/msr-repo-config.ttl`). The shape catalogue (`deploy/graphdb/msr-shapes.ttl` + generated `deploy/graphdb/msr-shapes-units.ttl`) matches this scope and additionally adds a **catalog-individual provenance shape** for `msr:MoltenSalt`/`msr:Constituent`/`msr:ChemicalCompound` (design D8, the same PROV invariant already required of measurements/mentions, extended to every pipeline-asserted individual). Bulk-load strategy is **per-transaction (incremental)**, confirmed by observed transaction sizes far below `transactionalValidationLimit` (500000) — see design D6. `ensure-repo.sh` fails loudly (not silently) on a pre-SHACL existing repo (design D7). Write-path rejections surface as `*graph.ValidationError` (`internal/graph/errors.go`), reported distinctly by `cmd/loader`.
