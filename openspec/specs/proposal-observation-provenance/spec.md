# proposal-observation-provenance Specification

## Purpose

Define how a change proposal records its corpus support as append-only per-document,
per-corpus `msr:Observation` nodes instead of a single stored document-frequency scalar, so
aggregates (document frequency, total occurrences, corpus count, corpora) are derived at read
time to exactly one value per proposal, cross-corpus breadth is surfaced as a reviewer signal,
and existing proposals can be backfilled deterministically without invoking the LLM triage step.
