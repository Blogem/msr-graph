...
Define a PROV-O-based provenance model for the knowledge graph: a self-contained PROV-O slice in the seed ontology, mandatory generation/derivation provenance on every pipeline-asserted instance individual, fully attributed generating activities (agent, timestamps, ontology version), and per-source/per-run named graphs holding those activity records via additive updates. The model lets any fact be attributed to who/what/when produced it without fabricating provenance.
