# safety-source-acquisition Specification

## Purpose

Define how the MSR safety-genre sources (IAEA SRS-123, the GIF/Holcomb MSR safety analysis, and the ORNL MSR technical-&-safety reports) are acquired into a gitignored cache under a tracked, attributed manifest, extracted and normalized into the pipeline's input format, and recorded as attributed `msr:Document` provenance nodes — without committing source PDFs or their full extracted text to the repository.
