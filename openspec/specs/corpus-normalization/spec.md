# corpus-normalization Specification

## Purpose

Define the deterministic, precision-biased OCR-normalization pre-pass and sentence segmentation applied to each curated document, producing `normalized.txt` and `segments.jsonl` artifacts with round-tripping character offsets that serve as the downstream extraction pipeline input.
