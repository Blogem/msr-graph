# qudt-unit-allowlist Specification

## Purpose

Define the vendored QUDT unit allowlist and the property→canonical-unit mapping used by the loader, and require that every emitted `hasUnit` IRI be validated against the allowlist before it is written to the graph.
