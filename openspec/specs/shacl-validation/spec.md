...
Define the declarative validation layer for the `msr` GraphDB repository: the SHACL shape catalogue (provenance + data-quality invariants), its residence in the reserved RDF4J shapes graph, commit-time enforcement on the graph write path (rejection with a legible report), and the bulk-load validation strategy.
