...
The `provenance-model` capability was subsequently **modified by `provenance-run-lineage`** (also landed and archived): the per-run `urn:msr:run:*` / per-source `urn:msr:src:*` named graphs were replaced by a single append-only `urn:msr:provenance` graph, and every fact now carries an *additional* per-run `prov:wasGeneratedBy` edge there (one per run) alongside the single stable `prov:wasGeneratedBy msrd:activity-<pipeline>` edge in `urn:msr:data`. This does **not** change these shapes: they require `prov:wasGeneratedBy` at minCount 1 with no maxCount, so the extra per-run edges over-satisfy the constraint rather than break it, and no shape targets the per-run `prov:Activity` audit nodes. The catalog-individual invariant the shapes enforce ("every pipeline-asserted individual carries generation + derivation provenance") is unchanged and still asserted by the current `provenance-model` spec.
...
- **Author the shape catalogue** and load it into the reserved RDF4J shapes graph (`http://rdf4j.org/schema/rdf4j#SHACLShapeGraph`):
  - *Provenance invariants*: `msr:PropertyMeasurement` requires `prov:wasDerivedFrom`, `prov:wasGeneratedBy`, `msr:dataLocator`, `msr:forProperty`, `msr:ofSalt`, `msr:hasUnit`, `msr:equationForm` (all minCount 1) — but **not** `msr:citedIn`, which no writer asserts yet (deferred to chunk 7, per the landed chunk 12 `provenance-model` design D3); `msr:Mention` requires `msr:inDocument`, `msr:startOffset`, `msr:endOffset`, `msr:surfaceForm`, `prov:wasDerivedFrom`, `prov:wasGeneratedBy`; and `msr:MoltenSalt`, `msr:Constituent`, `msr:ChemicalCompound` each require `prov:wasGeneratedBy` + `prov:wasDerivedFrom` (the full set of pipeline-asserted individuals provenance-model stamps, not just measurements/mentions).
