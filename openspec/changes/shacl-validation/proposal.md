...
This is chunk 13 of P3.5 and depends on chunk 12 `provenance-model`, which defines the PROV-O vocabulary (`prov:wasDerivedFrom`, `prov:wasGeneratedBy`, complete/required `msr:dataLocator`/`msr:citedIn`) the shapes encode. Shapes that require provenance predicates only make sense once those predicates are asserted by every writer.
...
- **Author the shape catalogue** and load it into the reserved RDF4J shapes graph (`http://rdf4j.org/schema/rdf4j#SHACLShapeGraph`):
  - *Provenance invariants*: `msr:PropertyMeasurement` requires `prov:wasDerivedFrom`, `msr:dataLocator`, `msr:citedIn`, `msr:forProperty`, `msr:ofSalt`, `msr:hasUnit`, `msr:equationForm` (all minCount 1); `msr:Mention` requires `msr:inDocument`, `msr:startOffset`, `msr:endOffset`, `msr:surfaceForm`.
...
- **Dependency**: requires the provenance vocabulary from chunk 12 `provenance-model` (not yet an active change). Shapes requiring provenance predicates will reject all writes until every writer emits them — sequence 12 → 13.
