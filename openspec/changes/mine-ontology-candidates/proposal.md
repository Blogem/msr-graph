...
- **Emit TBox-affecting candidates as proposals**: write the `ChangeProposal` resource +
  ...
  a `moderatedBy` property, the `graphite` individual, and the MSRE edge — mixed TBox +
  instance triples in one bundle). Any concrete `qk:`/`unit:` IRI a proposal asserts MUST
  come from chunk 2's vendored QUDT allowlist (`ontology/qudt-units.json`), else the proposal
  is rejected.
- **Instance-kind candidates bypass staging**: a new specific salt/compound/reactor under
  an *existing* class is written **directly to `urn:msr:data`** flagged `msr:autoAccepted`
  (provenance kept), never staged — *unless* it depends on proposed schema (e.g. `graphite`
  needs the proposed `Moderator` class), in which case it rides inside that proposal's
  bundle and reaches `urn:msr:data` only on approval.
...
- `instance-auto-accept`: write instance-kind candidates directly to `urn:msr:data` flagged
  `msr:autoAccepted` with provenance, except individuals that depend on proposed schema,
  which ride their proposal's bundle instead of auto-accepting.
...
- **New code**: novelty-mining modules in `extraction/src/msr_extraction/` (lexical
  ...
  writer, a `mine` CLI subcommand) plus a `pytest` suite under `extraction/tests/`.
...
- **Stores**: `urn:msr:staging` gains `msr:ChangeProposal` resources; `urn:msr:proposal/{id}`
  graphs gain proposed triples; `urn:msr:data` gains `msr:autoAccepted` individuals. No
  SQLite writes (text-derived *values* are chunk 7).
...
- **Depends on**: chunk 1 (`bootstrap-graph-infra` — stores, `urn:msr:staging`, core-dataset
  ...
  the graph reader). Transitively relies on chunk 2's salt catalog + QUDT allowlist and
  chunk 5's corpus, both reached through chunk 6. **Downstream**: produces the staging records
  chunk 9's governance API serves and promotes.
