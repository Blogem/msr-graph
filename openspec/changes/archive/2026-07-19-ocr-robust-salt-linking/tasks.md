...
- [x] 6.1 Rebuilt the extraction image and re-ran `link` over the full curated corpus (fresh `make up`→`load-nist`→`ingest`→`link`, no DeepSeek). Result: the anchor `LiF-BeF, (66-34 mole %)` resolves to `msrd:salt-BeF2-LiF-34.0-66.0` at layer 3, and the corpus goes from **0 → 15** salt-individual links (8407 `msr:Mention` triples in `urn:msr:data`, 15 targeting `MoltenSalt`); re-run is byte-identical (idempotent). `layer3 > 0` holds in every document that contains a composed salt loadable from the fluoride-only NIST catalog (ORNL-TM-2316, ORNL-4658, ORNL-TM-2256); the other 8 correctly show `layer3=0` (7 have no composed-salt candidate at all; ORNL-3293's only composed salts are chloride/nitrate, outside the fluoride catalog) — precision-safe, no fabricated links. (The original "`layer3 > 0` in every document" wording assumed every curated doc contains a loadable composed fluoride salt, which the corpus does not.) Known out-of-scope recall gap: a composition split across a segment boundary by a blank line (`LiF-BeF,-UF,` + `(70-10-20 mole %)` in ORNL-TM-2256) isn't captured as one candidate — a chunk-5 segmentation interaction. `ner-entity-linking` task 11.1 re-checked: the `layer3=0` anchor blocker is resolved.
