...
Define how linked mentions are written to the graph: a minimal mention vocabulary added to the seed TBox, and deterministic, blank-node-free mention instances written to `urn:msr:data` via additive SPARQL UPDATE, so re-running the pipeline adds no duplicates.
